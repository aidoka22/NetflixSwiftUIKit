...
class HomeViewController: UIViewController {
    ...
    func configureHeroHeaderUIView (){
        ...
        APICaller.shared.getTrendingMovies { [weak self] result in
            ...
            case .success(let titles):
                ...
                self?.headerView?.configure(with: TitleViewModel(titleName: selectedTitle?.original_name ?? selectedTitle?.original_name ?? "", posterURL: selectedTitle?.poster_path ?? ""))
    ...
    private func configureNavbar(){
        ...
        image  = image?.imageResized(to: CGSize(width: 20, height: 30))
